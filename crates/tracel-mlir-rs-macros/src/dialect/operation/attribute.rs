...
macro_rules! mlir_rs_attribute {
    ($name:ident) => {
        prefixed_string!("::mlir_rs::ir::attribute::", $name)
    };
}
...
static ATTRIBUTE_TYPES: LazyLock<HashMap<&'static str, &'static str>> = LazyLock::new(|| {
    ...
    macro_rules! initialize_attributes {
        ($($mlir:ident => $mlir_rs:ident),* $(,)*) => {
            $(
                map.insert(
                    mlir_attribute!($mlir),
                    mlir_rs_attribute!($mlir_rs),
                );
            )*
        };
    }
        ...
        Ok(Self {
            ...
            storage_type: syn::parse_str(
                ATTRIBUTE_TYPES
                    ...
                    .copied()
                    .unwrap_or(mlir_rs_attribute!(Attribute)),
            )?,
            storage_type_string,
...
impl OperationField for Attribute<'_> {
    ...
    fn add_arguments(&self, name: &Ident) -> TokenStream {
        ...
        quote! {
            &[(
                ::mlir_rs::ir::Identifier::new(self.context, #name_string),
                #name.into(),
            )]
        }
    }
